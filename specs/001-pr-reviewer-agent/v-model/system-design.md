...
|--------|------|-------------|---------------------|------|
| SYS-001 | Webhook Receiver | HTTPS endpoint `/api/webhook`; verifies `X-Hub-Signature-256` HMAC; ACKs within 2s and hands payloads to the pipeline asynchronously. | REQ-001, REQ-IF-002, REQ-NF-002 | Service |
| SYS-002 | Trigger Filter | Decides whether a PR or CI completion warrants review: self-account is author (`opened`/`ready_for_review`), requested reviewer, or `@review` command; applies `repo_config` + denylist; logs skips. | REQ-001, REQ-002, REQ-003 | Module |
| SYS-003 | Review Coordinator | Owns the queue, concurrency 1, CI-gate wait (`pending_ci`), dedup per (repo, PR, head SHA, model), re-review on new SHA, retry with backoff, idempotency-marker checks, partial-report handling, and the reconcile sweep. | REQ-004, REQ-010, REQ-014, REQ-NF-005, REQ-NF-006 | Subsystem |
...
| SYS-009 | Report Publisher | Reads the assembled review, posts the formal GitHub review (summary + CI status + scope/test findings + security results + model used, inline comments) with event `COMMENT` and the idempotency marker, and updates run state. | REQ-011, REQ-012, REQ-IF-001 | Service |
...
| SYS-011 | Comment Command Interpreter | Parses PR comment commands (`@review --model <alias>`); validates aliases; replies with help on unknown/malformed commands. | REQ-013 | Module |
...
| SYS-014 | Runtime & Observability | Process lifecycle (graceful shutdown, restart-safe recovery), structured JSON logs, persisted run records, GitHub rate-limit handling, secret hygiene, and read-only enforcement. | REQ-015, REQ-NF-001, REQ-NF-003, REQ-NF-004, REQ-NF-007, REQ-CN-002, REQ-CN-004 | Subsystem |
